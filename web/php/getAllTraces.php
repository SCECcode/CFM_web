...
$query = "SELECT OBJECT_tb.gid,OBJECT_tb.name,alternative,source_Author,CFM_version,model_description,descriptor,strike,dip,area,exposure,final_slip_sense,reference,reference_check,ID_comments,USGS_ID,AREA_tb.name,REGION_tb.name,SECTION_tb.name,FAULT_tb.name FROM OBJECT_tb, AREA_tb,REGION_tb, SECTION_tb, FAULT_tb where AREA_tb_gid = AREA_tb.gid and REGION_tb_gid = REGION_tb.gid and SECTION_tb_gid = SECTION_tb.gid and FAULT_tb_gid = FAULT_tb.gid";
